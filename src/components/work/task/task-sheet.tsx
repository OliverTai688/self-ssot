    ...
    <Sheet open={open} onOpenChange={(v) => !v && !isSaving && onClose()}>
      ...
      <SheetContent side="right" className="flex flex-col gap-0 p-0">
        <SheetHeader className="border-b p-4">
          <SheetTitle>{taskCopy.sheet.title}</SheetTitle>
        </SheetHeader>
        ...
        <div className="flex flex-col gap-4 flex-1 p-4">
          ...
          <div className="flex flex-col gap-1.5">
            <Label htmlFor="task-title">{taskCopy.sheet.name}</Label>
            <Input
              id="task-title"
              placeholder={taskCopy.sheet.namePlaceholder}
              value={title}
          ...
          <div className="flex flex-col gap-1.5">
            <Label>{taskCopy.sheet.priority}</Label>
            <Select
              value={priority}
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="1">{taskCopy.priorities[1]}</SelectItem>
                <SelectItem value="2">{taskCopy.priorities[2]}</SelectItem>
                <SelectItem value="3">{taskCopy.priorities[3]}</SelectItem>
              </SelectContent>
            </Select>
          </div>

          <div className="flex flex-col gap-1.5">
            <Label>{taskCopy.sheet.visibility}</Label>
            <Select
              value={visibility}
            ...
            >
              ...
              <SelectContent>
                <SelectItem value="internal">{taskCopy.visibility.internal}</SelectItem>
        ...
        <SheetFooter>
          ...
          >
            {isSaving ? taskCopy.sheet.saving : taskCopy.sheet.save}
          </Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
