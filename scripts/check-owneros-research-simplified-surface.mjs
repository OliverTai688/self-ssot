...
const RESEARCH_PAGE_PATH = "src/app/(dashboard)/research/page.tsx"
...
const REQUIRED_RESEARCH_PAGE_MARKERS = [
  "OWNEROS-UI-005-RESEARCH-SURFACE",
  ...
  "Research Operating Desk",
  "Organize sources, questions, evidence, outputs, and Research AI proposals.",
  "Prototype state",
  "Formal persistence pending",
  "Research Queue",
  "Research Readiness",
  "Source Evidence",
...
const REQUIRED_DOC_MARKERS = [
  TASK_ID,
  "research:simplified:check",
  "OWNEROS-UI-005-RESEARCH-SURFACE",
]

const FORBIDDEN_RESEARCH_PAGE_PATTERNS = [
  ...
  { label: "Research runtime DB write enabled", pattern: /research(?:Db|Database)Write(?:Allowed|Enabled)?\s*[:=]\s*true/ },
  { label: "Research runtime Prisma read enabled", pattern: /runtimePrismaReadEnabled\s*[:=]\s*true|livePrismaReadAllowed\s*[:=]\s*true/ },
]
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < filtered.length; index += 1) {
    ...
    if (arg === "--help" || arg === "-h") {
      console.log("Validate the simplified OwnerOS Research operating surface")
      console.log("")
      console.log("Usage:")
      console.log("  pnpm research:simplified:check")
      console.log("  pnpm research:simplified:check -- --json")
      console.log("  pnpm research:simplified:check -- --out docs/2_agent-input/generated/agent-loop/reports/<file>.json")
      process.exit(0)
    }
...
async function buildProof() {
  const issues = []
  const researchPage = await readText(RESEARCH_PAGE_PATH, issues)
  ...
  requireMarkers(researchPage, REQUIRED_RESEARCH_PAGE_MARKERS, RESEARCH_PAGE_PATH, issues, "RESEARCH_MARKER_MISSING")
  rejectForbidden(researchPage, FORBIDDEN_RESEARCH_PAGE_PATTERNS, RESEARCH_PAGE_PATH, issues)
  requireMarkers(packageJson, ['"research:simplified:check"'], PACKAGE_PATH, issues, "PACKAGE_SCRIPT_MISSING")
  ...
  return {
    ...
    route: "/research",
    researchPagePath: RESEARCH_PAGE_PATH,
    checkerCommand: "pnpm research:simplified:check",
    surface: {
      mode: "protected_owner_visible_prototype_research_module_surface",
      primaryJob: "Organize sources, questions, evidence, outputs, and Research AI proposals.",
      commandBarIncludes: ["New research", "Full Research list", "Source Evidence", "Readiness"],
      slots: [
        "identity-mode-strip",
        "attention-header",
        "command-bar",
    ...
    gateMapping: {
      gateA: "A4 Research owner path and A8 core no-mock usability prerequisite only",
...
async function main() {
  ...
  if (args.json) {
    ...
    console.log(`PASS ${TASK_ID}: simplified Research operating surface contract is present.`)
  } else {
    console.error(`FAIL ${TASK_ID}: simplified Research operating surface contract has ${proof.issues.length} issue(s).`)
