...
const SLUG = "operating-roundtrip-proof"
...
async function main() {
  const { applyOperatingCommands } = await import("../src/lib/services/operating-commands.service")
  const { loadOperatingStore } = await import("../src/lib/services/operating-store.service")

  const email = `roundtrip-${Date.now()}@example.invalid`
  const profile = await db.profile.create({ data: { email, fullName: "Roundtrip Proof", role: "OWNER" } })
  const workspace = await db.workspace.create({
    data: { type: "TEAM", name: "Roundtrip", slug: SLUG, createdByProfileId: profile.id },
  })

  try {
    const user = { id: profile.id, email, role: "OWNER" as const }
    const seat = { email, actor: "yz" as const, role: "owner" as const, canSwitchActor: false }

    // 寫入端與服務共用同一個 workspace slug，所以這次測試改走服務自己的解析；
    // 這裡先確認它挑到的是我們剛建立的那一個，否則後面的斷言會對錯資料。
    const baseVersion = 0
    ...
    const result = await applyOperatingCommands(user, seat, baseVersion, [
      {
        ...
        ent: "目標",
        label: "G-RT",
        changes: [{ collection: "goals", id: "G-RT", op: "create", after: { id: "G-RT", t: "整合測試目標", period: "2026 Q4", pct: 42 } }],
      },
      {
        clientRef: "rt-occasion",
        op: "create",
        ent: "活動",
        label: "OCC-RT",
        changes: [{ collection: "occasions", id: "OCC-RT", op: "create", after: { id: "OCC-RT", title: "月營運會議", cat: "公司活動", onDate: "2026-10-18", star: true } }],
      },
      {
        clientRef: "rt-txn",
        op: "create",
        ent: "交易",
        label: "T-RT",
        changes: [{ collection: "txns", id: "T-RT", op: "create", after: { id: "T-RT", d: "2026-10-02", t: "雲端主機", p: "PRJ-RT", cat: "工具", amt: -4200, pass: false, v: ["發票"], note: "月費" } }],
      },
      {
        clientRef: "rt-journal",
        op: "update",
        ent: "日誌",
        label: "2026-10-02",
        changes: [{ collection: "journal", id: "2026-10-02", op: "create", after: { title: "10/2", blocks: [{ id: "b1", t: "p", text: "整合測試" }], visibility: "company" } }],
      },
    ])
    ...
    const replay = await applyOperatingCommands(user, seat, result.version, [
      {
        ...
        ent: "目標",
        label: "G-RT",
        changes: [{ collection: "goals", id: "G-RT", op: "create", after: { id: "G-RT", t: "不該覆蓋", period: "x", pct: 0 } }],
      },
    ])
    eq("replaying a clientRef is accepted without reapplying", replay.applied, ["rt-goal"])
    eq("goal row count stays at one", await db.operatingGoal.count({ where: { workspaceId: workspace.id } }), 1)

    // store 是刻意的鬆散形狀（v5 runtime 的 DB），這裡只挑幾個欄位比對。
    type Row = Record<string, unknown>
    const store = (await loadOperatingStore(workspace.id, profile.id)) as Record<string, Row[] | Record<string, Row>>
    const rows = (key: string): Row[] => (Array.isArray(store[key]) ? (store[key] as Row[]) : [])

    const goal = rows("goals").find((g) => g.id === "G-RT")
    ...
    const occasion = rows("occasions").find((o) => o.id === "OCC-RT")
    ...
    const txn = rows("txns").find((t) => t.id === "T-RT")
    ...
    const entry = journal["2026-10-02"]
    check("journal day survives the round trip", Boolean(entry))
    eq("journal blocks survive", entry?.blocks, [{ id: "b1", t: "p", text: "整合測試" }])

    // 刪除也要能往返：少了這一條，delete 可能永遠沒被執行而測試仍然全綠。
    const deleted = await applyOperatingCommands(user, seat, replay.version, [
      {
        ...
        ent: "目標",
        label: "G-RT",
        changes: [{ collection: "goals", id: "G-RT", op: "delete" }],
      },
    ])
    eq("delete command applied", deleted.applied, ["rt-delete"])
    const afterDelete = (await loadOperatingStore(workspace.id, profile.id)) as Record<string, Row[]>
    check("deleted goal is gone on read", !(afterDelete.goals ?? []).some((g) => g.id === "G-RT"))
  } finally {
    await cleanup(workspace.id, profile.id)
    await db.$disconnect()
  }
