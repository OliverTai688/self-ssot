...
const save=async p=>{await p.locator('#drFoot').getByRole('button',{name:'儲存',exact:true}).click();await p.locator('#drawer').evaluate(e=>{if(e.classList.contains('on'))throw Error('Form did not close')})};
